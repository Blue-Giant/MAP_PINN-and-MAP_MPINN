...
def define_model_log_prob_bpinns(models, model_loss, data, tau_priors=None, tau_likes=None, predict=False,
    ...
    """This function defines the `log_prob_func` for torch nn.Modules. This will then be passed into the hamiltorch sampler.
    This is an important function for any work with Bayesian neural networks.
...
def update_paras_by_pinn(models, data, model_loss, learning_rate=0.1, show_learning_rate=True, update_lr=True,
                         step_cum2update_lr=100, step_gamma2update_lr=0.95, tau_priors=None, tau_likes=None,
                         store_on_GPU=True, device='cpu', n_params_single=None, pde=False,  total_epochs=10000,
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=step_cum2update_lr, gamma=step_gamma2update_lr)  # added by li xi'an on 2023.11.14
    for epoch in range(total_epochs+1):
        optimizer.zero_grad()
        loss = - log_prob_func(params)
        loss_list.append(loss.detach().item())              # added by li xi'an on 2024.05.27
        loss.backward(retain_graph=True)
        optimizer.step()
        if update_lr is True:
            scheduler.step()                # added by li xi'an on 2023.11.14
        if show_learning_rate is True:
            if epoch % 100 == 0:
...
# added by li xi'an on 2026.05.21
def update_paras_by_lxa_pinn(models, data, model_loss, learning_rate=0.1, show_learning_rate=True, update_lr=True,
                             step_cum2update_lr=100, step_gamma2update_lr=0.95, tau_priors=None, tau_likes=None,
                             total_epochs=10000):
    """Sample weights from a NN model to perform inference. This function builds a `log_prob_func` from the torch.nn.Module and passes it to `hamiltorch.sample`.
    Parameters
    ...
    dist = torch.distributions.Normal(0, tau_priors**-0.5)

    l_prior = 0.0
    for model in models:
        params_prior = model.parameters()
        for paras_init in params_prior:
            l_prior = dist.log_prob(paras_init).sum() + l_prior

    loss_list = []
    prior_scale = 1.0
    start_time = time.time()
    optimizer = torch.optim.Adam(models[0].parameters(), lr=learning_rate)
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=step_cum2update_lr, gamma=step_gamma2update_lr)  # added by li xi'an on 2023.11.14
    for epoch in range(total_epochs+1):
        optimizer.zero_grad()
        ll, output = model_loss(data, models, tau_likes)
        log_prob_func = -1.0 * (ll + l_prior / prior_scale)
        loss_list.append(log_prob_func.detach().item())              # added by li xi'an on 2024.05.27
        log_prob_func.backward(retain_graph=True)
        optimizer.step()
        if update_lr is True:
            scheduler.step()                # added by li xi'an on 2023.11.14
        if show_learning_rate is True:
            if epoch % 100 == 0:
                # modify by li xi'an on 2023.11.14
                print('Epoch: %d~~~~, loss: %.10f~~~~, learning rate: %.10f~~~~, time: %.2f' %
                      (epoch, log_prob_func.detach().item(), optimizer.param_groups[0]['lr'], time.time() - start_time))

    return models[0].parameters(), loss_list


# added by li xi'an on 2025.03.12
def pinn_update_paras_varying_train_set(models, data, model_loss, learning_rate=0.1, update_lr=True,
                                        step_cum2update_lr=100, step_gamma2update_lr=0.95, tau_priors=None,
                                        tau_likes=None, store_on_GPU=True, device='cpu',n_params_single=None, pde=False,
                                        current_epoch=100, params_init_val=None):
    """Sample weights from a NN model to perform inference. This function builds a `log_prob_func` from the torch.nn.Module and passes it to `hamiltorch.sample`.
    Parameters
    ----------
    models : list of torch.nn.Module(s)
    ...
    pde : bool
        Determines whether it is pde or not.
    current_epoch : int      modify by li xi'an on 2023.11.14  epochs-->total_epochs
        the current epoch when pinns is true
    ...
    if current_epoch % 100 == 0:
        # modify by li xi'an on 2023.11.14
        print('Epoch: %d~~~~, loss: %.10f~~~~, learning rate: %.10f~~~~, time: %.2f' %
              (current_epoch, loss.detach().item(), optimizer.param_groups[0]['lr'], time.time() - start_time))
...
# added by li xi'an on 2024.05.27
def update_paras_by_hamilton(
        models, data, model_loss, num_samples=10, step_num2per_sample=10, learning_rate=0.1, show_learning_rate=True,
        update_lr=True, step_cum2update_lr=100, step_gamma2update_lr=0.95, burn=0, inv_mass=None, jitter=None,
    ...
    return hamiltorch.sample(log_prob_func, params_init, num_samples=num_samples, show_learning_rate=show_learning_rate,
                             step_num2per_sample=step_num2per_sample, step_size=learning_rate, update_lr=update_lr,
                             step_cum2update_lr=step_cum2update_lr, step_gamma2update_lr=step_gamma2update_lr, burn=burn,
...
# added by li xi'an on 2025.03.12
def hamilton_update_paras__varying_train_set(models, data, model_loss, num_samples=10, num_steps_per_sample=10, learning_rate=0.1,
                        update_lr=True, step_cum2update_lr=100, step_gamma2update_lr=0.95, burn=0, inv_mass=None, jitter=None,
                        normalizing_const=1., softabs_const=None, explicit_binding_const=100,
                        fixed_point_threshold=1e-5, fixed_point_max_iterations=1000, jitter_max_tries=10,
                        sampler=hamiltorch.Sampler.HMC, integrator=hamiltorch.Integrator.IMPLICIT,
    ...
    return hamiltorch.sample(log_prob_func, params_init, num_samples=num_samples,
                             num_steps_per_sample=num_steps_per_sample, step_size=learning_rate, update_lr=update_lr,
                             step_cum2update_lr=step_cum2update_lr, step_gamma2update_lr=step_gamma2update_lr, burn=burn,
                             jitter=jitter, inv_mass=inv_mass, normalizing_const=normalizing_const,
                             softabs_const=softabs_const, explicit_binding_const=explicit_binding_const,
                             fixed_point_threshold=fixed_point_threshold,
...
def sample_model_bpinns(models, data, model_loss, num_samples=10, num_steps_per_sample=10, learning_rate=0.1,
    ...
    if pinns:  # ?
        ...
        return hamiltorch.sample(log_prob_func, params_init, num_samples=num_samples,
                                 ...
                                 step_cum2update_lr=step2change_lr, step_gamma2update_lr=gamma2change_lr, burn=burn,
                                 jitter=jitter, inv_mass=inv_mass, normalizing_const=normalizing_const,
                                 softabs_const=softabs_const, explicit_binding_const=explicit_binding_const,
                                 fixed_point_threshold=fixed_point_threshold,
                                 fixed_point_max_iterations=fixed_point_max_iterations,
...
def predict_model_by_pinn_lxa(models, data=None, model_loss=None, tau_likes=None):
    ll, output = model_loss(data, models, tau_likes)
    return output
