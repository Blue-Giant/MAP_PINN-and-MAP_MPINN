...
# where div is divergence operator(散度算子)， grad is gradient operator(梯度算子)
def model_loss2multiscale_sin(data, fmodel, params_unflattened, tau_likes, gradients, params_single=None, opt2device='cpu'):
    x_u = data["x_u"]
    y_u = data["y_u"]
    pred_u = fmodel[0](x_u, params=params_unflattened[0])
    ll = -0.5 * tau_likes[0] * ((pred_u - y_u) ** 2).sum(0)
...
#                  -->-[(da/dx)·(du/dx) + a·ddu/dxx + (da/dy)·(du/dy)+ a·ddu/dyy] = f
# where div is divergence operator(散度算子)， grad is gradient operator(梯度算子)
def model_loss2multiscale_eps0p5(data, fmodel, params_unflattened, tau_likes, gradients, params_single=None, opt2device='cpu'):
    x_u = data["x_u"]
    y_u = data["y_u"]
    pred_u = fmodel[0](x_u, params=params_unflattened[0])
    ll = -0.5 * tau_likes[0] * ((pred_u - y_u) ** 2).sum(0)
    ...
    aesp = 1.0/(4.0+torch.cos((2*torch.pi*x_f[:, 0:1]*x_f[:, 0:1]+2*torch.pi*x_f[:, 1:2]*x_f[:, 1:2])/0.5))

    femnu2apes = 4.0+torch.cos((2*torch.pi*x_f[:, 0:1]*x_f[:, 0:1]+2*torch.pi*x_f[:, 1:2]*x_f[:, 1:2])/0.5)
    da_dx = torch.sin((2*torch.pi*x_f[:, 0:1]*x_f[:, 0:1]+2*torch.pi*x_f[:, 1:2]*x_f[:, 1:2])/0.5)*(4.0*torch.pi*x_f[:, 0:1]/0.5)/(femnu2apes*femnu2apes)
    da_dy = torch.sin((2*torch.pi*x_f[:, 0:1]*x_f[:, 0:1]+2*torch.pi*x_f[:, 1:2]*x_f[:, 1:2])/0.5)*(4.0*torch.pi*x_f[:, 1:2]/0.5)/(femnu2apes*femnu2apes)
...
def model_loss2multiscale_eps0p2(data, fmodel, params_unflattened, tau_likes, gradients, params_single=None, opt2device='cpu'):
    x_u = data["x_u"]
    y_u = data["y_u"]
    pred_u = fmodel[0](x_u, params=params_unflattened[0])
    ll = -0.5 * tau_likes[0] * ((pred_u - y_u) ** 2).sum(0)
    ...
    aesp = 1.0/(4.0+torch.cos((2*torch.pi*x_f[:, 0:1]*x_f[:, 0:1]+2*torch.pi*x_f[:, 1:2]*x_f[:, 1:2])/0.2))

    femnu2apes = 4.0+torch.cos((2*torch.pi*x_f[:, 0:1]*x_f[:, 0:1]+2*torch.pi*x_f[:, 1:2]*x_f[:, 1:2])/0.2)
    da_dx = torch.sin((2*torch.pi*x_f[:, 0:1]*x_f[:, 0:1]+2*torch.pi*x_f[:, 1:2]*x_f[:, 1:2])/0.1)*(4.0*torch.pi*x_f[:, 0:1]/0.2)/(femnu2apes*femnu2apes)
    da_dy = torch.sin((2*torch.pi*x_f[:, 0:1]*x_f[:, 0:1]+2*torch.pi*x_f[:, 1:2]*x_f[:, 1:2])/0.1)*(4.0*torch.pi*x_f[:, 1:2]/0.2)/(femnu2apes*femnu2apes)
...
def model_loss2multiscale_eps0p1(data, fmodel, params_unflattened, tau_likes, gradients, params_single=None, opt2device='cpu'):
    x_u = data["x_u"]
    y_u = data["y_u"]
    pred_u = fmodel[0](x_u, params=params_unflattened[0])
    ll = -0.5 * tau_likes[0] * ((pred_u - y_u) ** 2).sum(0)
    ...
    aesp = 1.0/(4.0+torch.cos((2*torch.pi*x_f[:, 0:1]*x_f[:, 0:1]+2*torch.pi*x_f[:, 1:2]*x_f[:, 1:2])/0.1))

    femnu2apes = 4.0+torch.cos((2*torch.pi*x_f[:, 0:1]*x_f[:, 0:1]+2*torch.pi*x_f[:, 1:2]*x_f[:, 1:2])/0.1)
    da_dx = torch.sin((2*torch.pi*x_f[:, 0:1]*x_f[:, 0:1]+2*torch.pi*x_f[:, 1:2]*x_f[:, 1:2])/0.1)*(4.0*torch.pi*x_f[:, 0:1]/0.1)/(femnu2apes*femnu2apes)
    da_dy = torch.sin((2*torch.pi*x_f[:, 0:1]*x_f[:, 0:1]+2*torch.pi*x_f[:, 1:2]*x_f[:, 1:2])/0.1)*(4.0*torch.pi*x_f[:, 1:2]/0.1)/(femnu2apes*femnu2apes)
...
def solve_bayes_pinn(Rdic=None):
    ...
    if '0.1' == str.upper(Rdic['epsilon']):
        if 'PINN' == Rdic['mode2update_para']:
            params_hmc, losses = BNN_EstimatePara_Utils.update_paras_by_pinn(
                nets, data, model_loss=model_loss2multiscale_eps0p1, learning_rate=lr, update_lr=open_update_lr,
            ...
            params_hmc = BNN_EstimatePara_Utils.update_paras_by_hamilton(
                nets, data, model_loss=model_loss2multiscale_eps0p1, num_samples=num_samples, step_num2per_sample=L,
        ...
        pred_list, log_prob_list = BNN_EstimatePara_Utils.predict_model_bpinns(
            nets, samples=params_hmc, data=data_val, model_loss=model_loss2multiscale_eps0p1, tau_priors=tau_priors,
            tau_likes=tau_likes, pde=pde)
    elif '0.2' == str.upper(Rdic['epsilon']):
        if 'PINN' == Rdic['mode2update_para']:
            params_hmc, losses = BNN_EstimatePara_Utils.update_paras_by_pinn(
                nets, data, model_loss=model_loss2multiscale_eps0p2, learning_rate=lr, update_lr=open_update_lr,
                step_cum2update_lr=step2update_lr,
                step_gamma2update_lr=gamma2update_lr, tau_priors=tau_priors, tau_likes=tau_likes, device=device, pde=pde,
                total_epochs=max_epoch)
        else:
            params_hmc = BNN_EstimatePara_Utils.update_paras_by_hamilton(
                nets, data, model_loss=model_loss2multiscale_eps0p2, num_samples=num_samples, step_num2per_sample=L,
                learning_rate=lr, update_lr=open_update_lr, step_cum2update_lr=step2update_lr,
                step_gamma2update_lr=gamma2update_lr,
                burn=burn, tau_priors=tau_priors, tau_likes=tau_likes, device=device, pde=pde)

        pred_list, log_prob_list = BNN_EstimatePara_Utils.predict_model_bpinns(
            nets, samples=params_hmc, data=data_val, model_loss=model_loss2multiscale_eps0p2, tau_priors=tau_priors,
            tau_likes=tau_likes, pde=pde)
    else:
        if 'PINN' == Rdic['mode2update_para']:
            params_hmc, losses = BNN_EstimatePara_Utils.update_paras_by_pinn(
                nets, data, model_loss=model_loss2multiscale_eps0p5, learning_rate=lr, update_lr=open_update_lr,
                step_cum2update_lr=step2update_lr,
                step_gamma2update_lr=gamma2update_lr, tau_priors=tau_priors, tau_likes=tau_likes, device=device, pde=pde,
                total_epochs=max_epoch)
        else:
            params_hmc = BNN_EstimatePara_Utils.update_paras_by_hamilton(
                nets, data, model_loss=model_loss2multiscale_eps0p5, num_samples=num_samples, step_num2per_sample=L,
                learning_rate=lr, update_lr=open_update_lr, step_cum2update_lr=step2update_lr,
                step_gamma2update_lr=gamma2update_lr,
                burn=burn, tau_priors=tau_priors, tau_likes=tau_likes, device=device, pde=pde)

        pred_list, log_prob_list = BNN_EstimatePara_Utils.predict_model_bpinns(
            nets, samples=params_hmc, data=data_val, model_loss=model_loss2multiscale_eps0p5, tau_priors=tau_priors,
            tau_likes=tau_likes, pde=pde)
...
if __name__ == "__main__":
    ...
    store_file = 'BPINN_MultiScaleElliptic2D'
    ...
    # R['equa_name'] = 'Multiscale_eps0p5'
    R['equa_name'] = 'Multiscale_eps0p2'
    ...
    if R['mode2update_para'] == 'PINN':
        ...
        R['sample_num2hamilton'] = 100    # 执行100次Hamilton采样
        R['step_num2per_sample'] = 500    # 每次Hamilton采样迭代500轮
        R['burn'] = 10

        R['update_step_size'] = False
        R['init_step_size'] = 0.00001
        R['step2update_step_size'] = 20
        R['gamma2update_step_size'] = 0.95

    if R['equa_name'] == 'Multiscale_eps0p2':
        R['epsilon'] = '0.2'
    elif R['equa_name'] == 'Multiscale_eps0p5':
        R['epsilon'] = '0.5'
    else:
        R['epsilon'] = '0.1'
